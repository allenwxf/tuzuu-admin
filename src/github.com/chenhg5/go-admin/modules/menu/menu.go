...
func SetGlobalMenu(user auth.User) {
	...
	if user.IsSuperAdmin() {
		menus, _ = db.Query("default", "select * from goadmin_menu where id > 0 order by `order` asc")
	} else {
		...
		menus, _ = db.Query("default", "select * from goadmin_menu where id in ("+qmark[:len(qmark)-1]+") "+
...
func GetMenuItemById(id string) MenuItem {
	menu, _ := db.Query("default", "select * from goadmin_menu where id = ?", id)
