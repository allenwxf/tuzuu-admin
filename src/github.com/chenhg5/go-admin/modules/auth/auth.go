...
func Check(password string, username string) (user User, ok bool) {
	...
	admin, _ := db.Query("default", "select * from goadmin_users where username = ?", username)
	...
	if len(admin) < 1 {
		...
		if ComparePassword(password, admin[0]["password"].(string)) {
			...
			roleModel, _ := db.Query("default", "select r.id, r.name, r.slug from goadmin_role_users "+
			...
			db.Exec("default", "update goadmin_users set password = ? where id = ?", newPwd, user.ID)
		} else {
			ok = false
		}
	}
