...
func (driver *MysqlDriver) Load(sid string) map[string]interface{} {
	sesModel, _ := db.Query("default", "select * from goadmin_session where sid = ?", sid)
	if len(sesModel) < 1 {
...
func (driver *MysqlDriver) Update(sid string, values map[string]interface{}) {
	if sid != "" {
		if len(values) == 0 {
			db.Exec("default", "delete from goadmin_session where sid = ?", sid)
			return
		}
		valuesByte, _ := json.Marshal(values)
		sesModel, _ := db.Query("default", "select * from goadmin_session where sid = ?", sid)
		if len(sesModel) < 1 {
			db.Exec("default", "insert into goadmin_session (`values`, sid) values (?, ?)", string(valuesByte), sid)
		} else {
			db.Exec("default", "update goadmin_session set `values` = ? where sid = ?", string(valuesByte), sid)
		}
	}
}
