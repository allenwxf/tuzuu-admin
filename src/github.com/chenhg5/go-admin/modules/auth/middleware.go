...
func GetCurUserById(id string) (user User, ok bool) {
	admin, _ := db.Query("default", "select * from goadmin_users where id = ?", id)
	...
	roleModel, _ := db.Query("default", "select r.id, r.name, r.slug from goadmin_role_users as u "+
	...
	menuIdsModel, _ := db.Query("default", "select menu_id, parent_id from goadmin_role_menu left join "+
...
func GetPermissions(role_id interface{}) []map[string]interface{} {
	permissions, _ := db.Query("default", "select p.http_method, p.http_path from goadmin_role_permissions "+
