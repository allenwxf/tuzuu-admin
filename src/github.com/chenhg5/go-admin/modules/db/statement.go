...
func (sql *Sql) Find(con string, arg interface{}) (map[string]interface{}, error) {
	return sql.Where("id", "=", arg).First(con)
}

func (sql *Sql) Count(con string) (int64, error) {
	var (
		res map[string]interface{}
		err error
	)
	if res, err = sql.Select("count(*)").First(con); err != nil {
		return 0, err
	}
...
func (sql *Sql) First(con string) (map[string]interface{}, error) {
	...
	res, _ := GetConnection().Query(con, sql.statement, sql.args...)
...
func (sql *Sql) All(con string) ([]map[string]interface{}, error) {
	defer RecycleSql(sql)
...
func (sql *Sql) Update(con string, values H) (int64, error) {
	defer RecycleSql(sql)

	sql.prepareUpdate(values)

	res := GetConnection().Exec(con, sql.statement, sql.args...)
...
func (sql *Sql) Delete(con string) error {
	defer RecycleSql(sql)
...
func (sql *Sql) Exec(con string) (int64, error) {
	defer RecycleSql(sql)
...
func (sql *Sql) Insert(con string, values H) (int64, error) {
	defer RecycleSql(sql)
