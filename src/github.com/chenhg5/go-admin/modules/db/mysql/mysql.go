...
func (db *Mysql) Query(con string, query string, args ...interface{}) ([]map[string]interface{}, *sql.Rows) {
	return performer.Query(db.SqlDBmap[con], query, args...)
}

func (db *Mysql) Exec(con string, query string, args ...interface{}) sql.Result {
	return performer.Exec(db.SqlDBmap[con], query, args...)
}

func (db *Mysql) BeginTransactionsWithReadUncommitted(con string) *SqlTxStruct {
	return db.BeginTransactionsWithLevel(con, sql.LevelReadUncommitted)
}

func (db *Mysql) BeginTransactionsWithReadCommitted(con string) *SqlTxStruct {
	return db.BeginTransactionsWithLevel(con, sql.LevelReadCommitted)
}

func (db *Mysql) BeginTransactionsWithRepeatableRead(con string) *SqlTxStruct {
	return db.BeginTransactionsWithLevel(con, sql.LevelRepeatableRead)
}

func (db *Mysql) BeginTransactions(con string) *SqlTxStruct {
	return db.BeginTransactionsWithLevel(con, sql.LevelDefault)
}

func (db *Mysql) BeginTransactionsWithLevel(con string, level sql.IsolationLevel) *SqlTxStruct {
	tx, err := db.SqlDBmap[con].BeginTx(context.Background(),
...
func (db *Mysql) WithTransaction(con string, fn TxFn) (err error, res map[string]interface{}) {

	SqlTx := db.BeginTransactions(con)
...
func (db *Mysql) WithTransactionByLevel(con string, level sql.IsolationLevel, fn TxFn) (err error, res map[string]interface{}) {

	SqlTx := db.BeginTransactionsWithLevel(con, level)
