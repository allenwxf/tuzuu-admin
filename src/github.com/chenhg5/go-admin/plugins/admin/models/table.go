...
func (tb Table) GetDataFromDatabase(con string, path string, params *Parameters) PanelInfo {
	...
	columnsModel, _ := tb.db().Query(con, showColumns)
	...
	res, _ := tb.db().Query(con, "select "+fields+" from "+tb.Info.Table+wheres+" order by "+params.SortField+" "+
	...
	total, _ := tb.db().Query(con, "select count(*) from "+tb.Info.Table+wheres, whereArgs...)
	var size int
	if tb.ConnectionDriver == "sqlite" {
...
func (tb Table) GetDataFromDatabaseWithId(con string, id string) ([]types.Form, string, string) {

	fields := ""

	columnsModel, _ := tb.db().Query(con, "show columns in " + tb.Form.Table)
	columns := GetColumns(columnsModel, tb.ConnectionDriver)
	...
	res, _ := tb.db().Query(con, "select "+fields+" from "+tb.Form.Table+" where id = ?", id)
...
func (tb Table) UpdateDataFromDatabase(con string, dataList map[string][]string) {
	...
	tb.db().Exec(con, "update "+tb.Form.Table+" set "+fields+" where id = ?", valueList...)
}

// InsertDataFromDatabase insert data.
func (tb Table) InsertDataFromDatabase(con string, dataList map[string][]string) {
	...
	tb.db().Exec(con, "insert into "+tb.Form.Table+"("+fields+") values ("+queStr+")", valueList...)
}

// DeleteDataFromDatabase delete data.
func (tb Table) DeleteDataFromDatabase(con string, id string) {
	idArr := strings.Split(id, ",")
	for _, id := range idArr {
		tb.db().Exec(con, "delete from "+tb.Form.Table+" where id = ?", id)
	}
	if tb.Form.Table == "goadmin_roles" {
		tb.db().Exec(con, "delete from goadmin_role_users where role_id = ?", id)
		tb.db().Exec(con, "delete from goadmin_role_permissions where role_id = ?", id)
		tb.db().Exec(con, "delete from goadmin_role_menu where role_id = ?", id)
	}
	if tb.Form.Table == "goadmin_users" {
		tb.db().Exec(con, "delete from goadmin_role_users where user_id = ?", id)
		tb.db().Exec(con, "delete from goadmin_user_permissions where user_id = ?", id)
	}
	if tb.Form.Table == "goadmin_permissions" {
		tb.db().Exec(con, "delete from goadmin_role_permissions where permission_id = ?", id)
		tb.db().Exec(con, "delete from goadmin_user_permissions where permission_id = ?", id)
	}
	if tb.Form.Table == "goadmin_menu" {
		tb.db().Exec(con, "delete from goadmin_role_menu where menu_id = ?", id)
	}
}
