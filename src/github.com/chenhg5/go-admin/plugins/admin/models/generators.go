...
func GetManagerTable() (ManagerTable Table) {
	...
	ManagerTable.Info.FieldList = []types.Field{
		...
		},
		{
			...
			Sortable: false,
			ExcuFun: func(model types.RowModel) interface{} {
				labelModel, _ := db.Query("default", "select r.name from goadmin_role_users as u left join goadmin_roles as r on "+
	...
	rolesModel, _ := db.Query("default", "select `id`, `slug` from goadmin_roles where id > ?", 0)
	...
	permissionsModel, _ := db.Query("default", "select `id`, `slug` from goadmin_permissions where id > ?", 0)
	...
	ManagerTable.Form.FormList = []types.Form{
		{
			...
			Options:  roles,
			ExcuFun: func(model types.RowModel) interface{} {
				roleModel, _ := db.Query("default", "select role_id from goadmin_role_users where user_id = ?", model.ID)
				var roles []string
			...
			Options:  permissions,
			ExcuFun: func(model types.RowModel) interface{} {
				permissionModel, _ := db.Query("default", "select permission_id from goadmin_user_permissions where user_id = ?", model.ID)
	...
	RolesTable.Form.FormList = []types.Form{
		{
			...
			Options:  permissions,
			ExcuFun: func(model types.RowModel) interface{} {
				perModel, _ := db.Query("default", "select permission_id from goadmin_role_permissions where role_id = ?", model.ID)
				var permissions []string
...
func GetMenuTable() (MenuTable Table) {
	...
	parentsModel, _ := db.Query("default", "select `id`, `title` from goadmin_menu where id > ? order by `order` asc", 0)
	...
	MenuTable.Form.FormList = []types.Form{
		{
			...
			Options:  parents,
			ExcuFun: func(model types.RowModel) interface{} {
				menuModel, _ := db.Query("default", "select parent_id from goadmin_menu where id = ?", model.ID)
				var menuItem []string
			...
			Options:  roles,
			ExcuFun: func(model types.RowModel) interface{} {
				roleModel, _ := db.Query("default", "select role_id from goadmin_role_menu where menu_id = ?", model.ID)
				var roles []string
				for _, v := range roleModel {
					roles = append(roles, strconv.FormatInt(v["role_id"].(int64), 10))
				}
