...
func ShowEditMenu(ctx *context.Context) {
	...
	formData, title, description := models.TableList["menu"].GetDataFromDatabaseWithId("default", ctx.Query("id"))
...
func DeleteMenu(ctx *context.Context) {
	...
	db.Exec("default", "delete from goadmin_menu where id = ?", ctx.Query("id"))
...
func EditMenu(ctx *context.Context) {
	...
	for _, roleId := range roles {
		checkRoleMenu, _ := db.Query("default", "select * from goadmin_role_menu where role_id = ? and menu_id = ?", roleId, id)
		if len(checkRoleMenu) < 1 {
			db.Exec("insert into goadmin_role_menu (menu_id, role_id) values (?, ?)", id, roleId)
		}
	}

	db.Exec("default", "update goadmin_menu set title = ?, parent_id = ?, icon = ?, uri = ? where id = ?",
...
func NewMenu(ctx *context.Context) {
	...
	res := db.Exec("default", "insert into goadmin_menu (title, parent_id, icon, uri, `order`) values (?, ?, ?, ?, ?)",
	...
	for _, roleId := range roles {
		db.Exec("default", "insert into goadmin_role_menu (menu_id, role_id) values (?, ?)", id, roleId)
	}
...
func MenuOrder(ctx *context.Context) {
	...
	for _, v := range data {
		if child, ok := v["children"]; ok {
			db.Exec("default", "update goadmin_menu set `order` = ? where id = ?", count, v["id"])
			for _, v2 := range child.([]interface{}) {
				db.Exec("default", "update goadmin_menu set `order` = ? where id = ?", count, v2.(map[string]interface{})["id"])
				count++
			}
		} else {
			db.Exec("default", "update goadmin_menu set `order` = ? where id = ?", count, v["id"])
