...
func ShowForm(ctx *context.Context) {
	...
	formData, title, description := models.TableList[prefix].
		GetDataFromDatabaseWithId("default", ctx.Query("id"))
...
func EditForm(ctx *context.Context) {
	...
	if prefix == "manager" { // 管理员管理编辑
		...
		models.TableList[prefix].UpdateDataFromDatabase("default", (*form).Value)
	}
	...
	panelInfo := models.TableList[prefix].GetDataFromDatabase("default", prevUrlArr[0], params)
